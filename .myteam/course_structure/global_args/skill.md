...
Both `global_args.yaml` and `global-args.yaml` are common; either works since the filename is passed explicitly via
`--global-args`.
...
# Run with: mdxcanvas --course-info course_info.json --global-args global_args.yaml course.canvas.md.xml.jinja
...
Note: `--global-args` applies to the entire course entry point and all its `<include>` files. Use `--args` (not
