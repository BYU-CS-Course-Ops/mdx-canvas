...
The `<question>` tag is used within the `<questions>` section of a `<quiz>`. Each question has a `type` attribute; the
tag body is the question prompt.
...
Override the default point value per question. Use `points="0"` for checklist-style questions that appear in a quiz but
carry no grade weight:
