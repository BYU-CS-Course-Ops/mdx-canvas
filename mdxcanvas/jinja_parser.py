...
def get_global_args(global_args_path: Path) -> dict:
...
def process_csv(template_file: Path, args_path: Path, global_args_path: Path = None, line_id: str = None, **kwargs) -> str:
    template = template_file.read_text()
    arg_sets = list(csv.DictReader(args_path.read_text().splitlines()))

    if global_args_path:
        global_args = get_global_args(global_args_path)
...
if __name__ == '__main__':
    ...
    print(process_csv(template_file=test_material_path / "test.canvas.xml.jinja",
